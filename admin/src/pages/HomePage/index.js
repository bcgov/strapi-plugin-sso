    ...
    <CheckPermissions permissions={[{ action: 'plugin::strapi-plugin-sso.read', subject: null }]}>
      ...
      <HeaderLayout
        ...
        subtitle={formatMessage({
          ...
          defaultMessage:
            "Allowed default roles after signup via sso (note that checked roles don't mean that each user will have those by default, they also need to have that specific role in they user.roles object set by auth provider)",
        })}
      />
      <Box padding={10}>
        {showSuccess && (
          <AlertMessage>
            <Alert title="Success" variant={'success'} closeLabel={''} onClose={() => setSuccess(false)}>
              {formatMessage({
        ...
        {showError && (
          <AlertMessage>
            <Alert title="Error" variant={'danger'} closeLabel={''} onClose={() => setError(false)}>
              {formatMessage({
